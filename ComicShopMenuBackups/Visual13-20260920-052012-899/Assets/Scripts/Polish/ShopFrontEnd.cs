...
public sealed partial class ShopFrontEnd : MonoBehaviour
{
    ...
    void BuildSettings()
    {
        ...
        if (tab == 0)
        {
            Slider(content, "Ana ses", 228, 0, 1, p.master, value => p.master = value);
            Slider(content, "Kitaplar, adımlar & arayüz", 307, 0, 1, p.effects, value => p.effects = value);
            Slider(content, "Dükkân ortamı", 386, 0, 1, p.ambience, value => p.ambience = value);
            Slider(content, "Menü müziği", 465, 0, 1, p.music, value => p.music = value);
            Button(content, "Etkileşim ipuçları: " + (p.hints ? "AÇIK" : "KAPALI"), 0, 550, 708, 40, () => { p.hints = !p.hints; ShopSettings.Apply(); Build(); });
        ...
        Button(content, "KAYDET & GERİ", 368, 644, 340, 48, BackFromSettings, true);
    }
    void BuildResults()
    {
