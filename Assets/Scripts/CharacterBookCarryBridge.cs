...
public sealed class CharacterBookCarryBridge : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Q writes a world-space pose. Keep that book out of the animated hand
        // hierarchy so subsequent socket motion cannot shift or shake it again.
        if (interaction.IsThrowPoseActive)
        {
            var throwingBook = interaction.ActiveHeldBook;
            if (throwingBook != null && throwingBook.transform.IsChildOf(anchor))
                throwingBook.transform.SetParent(null, true);
        }
        ApplyAnchorPose();
    }
